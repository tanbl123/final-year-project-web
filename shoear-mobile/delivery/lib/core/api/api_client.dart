...
/// Thin HTTP wrapper around the PHP API. Every response is the envelope
/// { success, data, error }, so we unwrap `data` on success and throw
/// [ApiException] with the server's message otherwise.
class ApiClient {
  ...
  Future<dynamic> patch(String path, Map<String, dynamic> body) async {
    final uri = Uri.parse('$baseUrl$path');
    final res = await http.patch(uri, headers: _headers(json: true), body: jsonEncode(body));
    return _unwrap(res);
  }
